...
class BaseScraper:
    ...
    def __init__(
        ...
        self.REQUEST_DIR: Path = self.CWD / Path(
            f"data/scraped/{self.SITE_NAME}/requests"
        )
        self.SUBREQUEST_DIR: Path = self.CWD / Path(
            f"data/scraped/{self.SITE_NAME}/sub_requests"
        )
    ...
    async def handle_abstr_request(
        self, request_obj, request_dir, extract_request_info
    ):
    ...
    async def handle_request(self, request_obj):
        return await self.handle_abstr_request(
            request_obj, self.REQUEST_DIR, self.extract_request_info
        )

    async def handle_sub_request(self, sub_request_obj):
        return await self.handle_abstr_request(
            sub_request_obj, self.SUBREQUEST_DIR, self.extract_sub_request_info
        )
    ...
    def handle_job_request(self, *args, **kwds):
        raise NotImplementedError(self.NOT_IMPLEMENTED_MSG)
